...
const REPEATED_CALLS_EXCLUDED_NAMES = new Set(['ERC1967Proxy', 'FiatTokenProxy'])
...
export function detectPatterns(
  ...
  // MEV: 3+ txs in same block with value > 0, OR consecutive txs within 3s to different contracts
  ...
  const sameBlockHighValue = [...blockTxMap.values()].some(
    (list) => list.length >= 3 && list.some((tx) => BigInt(tx.value || '0') > BigInt(0)),
  )
  const addrTxsSorted = txs
    .filter((tx) => tx.timestamp && tx.from.hash.toLowerCase() === addrLower)
    .sort((a, b) => new Date(a.timestamp!).getTime() - new Date(b.timestamp!).getTime())
  let sandwichPattern = false
  for (let i = 0; i + 1 < addrTxsSorted.length; i++) {
    const t1 = new Date(addrTxsSorted[i].timestamp!).getTime()
    const t2 = new Date(addrTxsSorted[i + 1].timestamp!).getTime()
    if (t2 - t1 <= 3000) {
      const to1 = addrTxsSorted[i].to?.hash?.toLowerCase()
      const to2 = addrTxsSorted[i + 1].to?.hash?.toLowerCase()
      if (to1 && to2 && to1 !== to2) { sandwichPattern = true; break }
    }
  }
  const mev = sameBlockHighValue || sandwichPattern
  ...
  const repeatedCalls = [...callCounts.values()].some((c) => c > repeatThreshold)
