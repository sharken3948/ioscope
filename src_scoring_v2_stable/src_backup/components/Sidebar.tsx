  ...
  return (
    ...
    >
      ...
      <nav className="flex flex-col p-2 gap-0.5 flex-1">
        <div className="px-3 py-2 text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--text)', opacity: 0.3 }}>
          Analysis
        </div>
        {NAV_ITEMS.map((item) => (
          <a
            key={item.label}
            href={item.href}
            className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors hover:opacity-100"
        ...
        <div className="px-3 py-2 text-xs font-semibold uppercase tracking-widest mt-4" style={{ color: 'var(--text)', opacity: 0.3 }}>
          Tools
        </div>
        {EXTRA_NAV.map((item) => (
          <Link
